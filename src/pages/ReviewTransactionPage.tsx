...
export function ReviewTransactionPage() {
  ...
  const supplierName = id === 'SUP-338' ? 'Green Delta Farms' : 'Adeyemi Produce Co.';
  const pickupLocation = id === 'SUP-338' ? 'Ilorin, Kwara State' : 'Ogbomoso, Oyo State';
  const unitPrice = id === 'SUP-338' ? 465000 : 480000;
  const quantity = 12;
  const goodsSubtotal = unitPrice * quantity; // 5,760,000
  const logisticsCost = 185000;
  const platformFee = Math.round(goodsSubtotal * 0.01); // 57,600
  ...
  const handleSendRequest = async () => {
    ...
    try {
      ...
      toast('success', 'Request sent to supplier. Status: PENDING_SUPPLIER_ACCEPTANCE');
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5 shadow-xs">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-xs">
              <div>
                ...
                <div className="font-semibold text-gray-900 mt-0.5">White Maize</div>
              </div>
              <div>
                <div className="text-gray-500">Grade</div>
                <div className="font-semibold text-gray-900 mt-0.5">Grade A</div>
              </div>
              <div>
                <div className="text-gray-500">Quantity</div>
                <div className="font-semibold text-gray-900 mt-0.5">{quantity} tonnes</div>
              </div>
              <div>
                <div className="text-gray-500">Unit price</div>
                <div className="font-semibold text-gray-900 mt-0.5">₦{unitPrice.toLocaleString()} / tonne</div>
              </div>
            </div>
          </div>
