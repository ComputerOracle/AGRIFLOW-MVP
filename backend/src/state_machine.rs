...
impl TransactionStatus {
    ...
    pub fn allowed_transitions(&self) -> &'static [TransactionStatus] {
        ...
        match self {
            ...
            LogisticsPending => &[LogisticsAssigned, LogisticsAccepted, ReadyForPickup],
            LogisticsAssigned => &[LogisticsAccepted, LogisticsRejected, ReadyForPickup],
            LogisticsAccepted => &[ReadyForPickup, PickedUp],
            LogisticsRejected => &[LogisticsPending],
            ReadyForPickup => &[PickedUp, InTransit],
            PickedUp => &[InTransit, Delivered],
            InTransit => &[Delivered, BuyerConfirmationPending, DeliveryFailed],
            Delivered => &[BuyerConfirmationPending, DeliveryConfirmed, Completed, Disputed],
            BuyerConfirmationPending => &[Completed, Disputed],
    ...
    pub fn allowed_actors(&self) -> Option<&'static [Actor]> {
        ...
        match self {
            ...
            PaymentPending => Some(&[Buyer]),
            PaymentConfirmed => Some(&[System]),
            PaymentFailed => Some(&[System]),
            PaymentCancelled => Some(&[Buyer]),
            LogisticsPending => Some(&[System]),
            LogisticsAssigned => Some(&[Admin]),
            LogisticsAccepted => Some(&[Logistics]),
            LogisticsRejected => Some(&[Logistics]),
            ReadyForPickup => Some(&[Logistics]),
            PickedUp => Some(&[Logistics]),
            InTransit => Some(&[Logistics]),
            Delivered => Some(&[Logistics]),
            BuyerConfirmationPending => Some(&[Logistics, System]),
            DeliveryConfirmed => Some(&[Buyer]),
            DeliveryFailed => Some(&[Buyer, Logistics]),
